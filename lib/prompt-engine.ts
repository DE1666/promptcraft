export type Category = 'visual' | 'code' | 'content'
...
export function settingsForCategory(category: Category): Settings {
  return { ...initialSettings, category, idea: '', engine: categories[category].engines[0], style: categories[category].styles[0], flags: category === 'visual' ? ['Aspect ratio', 'Negative prompt'] : category === 'code' ? ['Accessibility', 'Include tests'] : ['Call to action'] }
}
...
export function buildPrompt(settings: Settings, variant = 0): { text: string; score: number } {
  ...
  if (category === 'visual') {
    ...
    const params = engine === 'Midjourney v6' ? `${flags.includes('Aspect ratio') ? `--ar ${ratio} ` : ''}--v 6 --style raw --s ${[250, 180, 320][variant % 3]}` : `${engine} · ${flags.includes('Aspect ratio') ? `${ratio} aspect ratio · ` : ''}${style} treatment`
    add('PARAMETERS', params)
  } else if (category === 'code') {
    add('ROLE & OBJECTIVE', `Act as a senior product engineer working in ${engine}.\n${idea.trim()}`)
    ...
    if (flags.includes('Stack constraints')) add('TECH STACK', 'Next.js App Router, React, TypeScript, Tailwind CSS, and PostgreSQL. Use server components by default and parameterized database queries. Keep secrets server-side.')
    ...
    if (flags.includes('Include tests')) add('TESTING', 'Include unit tests for business logic and integration tests for the primary user flow. Cover input validation, permission boundaries, and failure states.')
